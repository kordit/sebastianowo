 ...
 * z odpowiednią walidacją i komunikatami zwrotnymi
 */
class ManagerUser
{
    /**
    ...
    {
        // Endpoint do aktualizacji pól użytkownika
        register_rest_route($this->namespace, '/update-user-field', [
            'methods' => 'POST',
            'callback' => [$this, 'rest_update_user_field'],
            'permission_callback' => [$this, 'check_permissions'],
        ]);

        // Endpoint do pobierania danych użytkownika
        register_rest_route($this->namespace, '/get-user-data', [
            'methods' => 'GET',
     ...
     * @return WP_REST_Response Odpowiedź REST API
     */
    public function rest_update_user_field($request)
    {
        ...
        if ($field_type === 'area') {
            ...
            if (empty($action) || empty($area_id)) {
                return new WP_REST_Response([
                    'success' => false,
                    'message' => 'Brak wymaganych parametrów dla rejonu'
                ], 400);
            }

            $result = [];
            if ($action === 'add') {
                $result = $this->addAvailableArea($area_id);
            } else if ($action === 'remove') {
                $result = $this->removeAvailableArea($area_id);
            } else if ($action === 'set_current') {
                $result = $this->setCurrentArea($area_id);
            } else {
                $result = [
                    'success' => false,
                    'message' => 'Nieznana operacja na rejonach'
                ];
            }
        ...
        // Obsługa przedmiotów - specjalne parametry
        if ($field_type === 'item') {
            $item_id = intval($request->get_param('itemId'));
            $quantity = intval($request->get_param('quantity'));

            if (empty($field_name) || empty($item_id) || $quantity <= 0) {
                return new WP_REST_Response([
                    'success' => false,
                    'message' => 'Brak wymaganych parametrów dla przedmiotu'
                ], 400);
            }

            $result = [];
            if ($field_name === 'add') {
                $result = $this->addItem($item_id, $quantity);
            } else if ($field_name === 'remove') {
                $result = $this->removeItem($item_id, $quantity);
            } else {
                $result = [
                    'success' => false,
                    'message' => 'Nieznana operacja na przedmiotach'
                ];
            }
        ...
        switch ($field_type) {
            case 'stat':
                $result = $this->updateStat($field_name, $value);
                break;
            case 'skill':
                $result = $this->updateSkill($field_name, $value);
                break;
            case 'backpack':
                $result = $this->updateBackpack($field_name, $value);
                break;
            case 'vitality':
                $result = $this->updateVitality($field_name, $value);
                break;
            case 'progress':
                $result = $this->updateProgress($field_name, $value);
                break;
            default:
                $result = $this->updateNumericField($field_name, $value);
                break;
        }
    ...
    /**
     * Obsługa endpoint REST API do pobierania danych użytkownika
     * 
     * @param WP_REST_Request $request Obiekt żądania
     * @return WP_REST_Response Odpowiedź REST API
     */
    public function rest_get_user_data($request)
    {
        ...
        return new WP_REST_Response(['success' => true, 'data' => $user_data], 200);
    }
    ...
    public function updateNumericField($field_name, $value, $label = null)
    {
        ...
        // Przygotuj etykietę dla komunikatu
    ...
    public function removeItem($item_id, $quantity)
    {
        ...
        // Jeśli nie ma żadnych przedmiotów, zwróć błąd
        if (!$items) {
            return [
                'success' => false,
        ...
        // Sprawdź, czy użytkownik posiada ten przedmiot w wystarczającej ilości
        $found = false;
        foreach ($items as $key => $user_item) {
            if ($user_item['item']->ID == $item_id) {
                // Przedmiot znaleziony, sprawdź ilość
                if ($user_item['quantity'] < $quantity) {
                    return [
                        'success' => false,
                        'message' => "Nie masz wystarczającej ilości {$item->post_title} w plecaku"
                    ];
                }
        ...
        // Jeśli przedmiot nie został znaleziony, zwróć błąd
        if (!$found) {
            return [
                'success' => false,
                'message' => 'Nie masz tego przedmiotu w plecaku'
