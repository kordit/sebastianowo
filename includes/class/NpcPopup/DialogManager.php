...
class DialogManager {
    ...
    public function get_first_matching_dialog(array $dialogs, array $criteria): ?array {
        // Aktualizuj kontekst na podstawie kryteriów
        ...
        foreach ($dialogs as $dialog) {
            ...
            // Sprawdzenie warunków dialogu
            $conditions = $dialog['conditions'] ?? [];
            if (!$condition_checker->check_conditions($conditions)) {
                $dialog_id = $dialog['dialog_id'] ?? 'nieznany';
                $this->logger->log("Dialog {$dialog_id} niedostępny - warunki niespełnione", 'info');
                continue;
            }
            
            $this->logger->log("Znaleziono pasujący dialog: " . ($dialog['dialog_id'] ?? 'nieznany'), 'info');
            return $dialog;
        }
    ...
    public function simplify_dialog(array $dialog): array {
        $this->logger->log("Upraszczanie struktury dialogu: " . print_r($dialog, true), 'debug');
        ...
        $this->logger->log("Uproszczona struktura dialogu: " . print_r($simplified, true), 'debug');
     ...
     * @param array $dialogs Lista wszystkich dialogów NPC
     * @param ConditionChecker $condition_checker Obiekt sprawdzający warunki
     * @return array|null Dialog lub null, jeśli nie znaleziono
     */
    public function get_dialog(string $dialog_id, array $dialogs, ConditionChecker $condition_checker): ?array {
        foreach ($dialogs as $dialog) {
            if (!isset($dialog['dialog_id']) || $dialog['dialog_id'] != $dialog_id) {
                continue;
            }
            
            // Sprawdź, czy warunki dialogu są spełnione
            $conditions = $dialog['conditions'] ?? [];
            if (!$condition_checker->check_conditions($conditions)) {
                $this->logger->log("Dialog {$dialog_id} niedostępny - warunki niespełnione", 'info');
                continue;
        ...
        $this->logger->log("Dialog {$dialog_id} nie istnieje lub jest niedostępny", 'warning');
        return null;
    }
