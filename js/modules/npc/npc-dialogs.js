...
window.NpcDialogs = (function () {
            ...
            case 'otworz_chat':
                ...
                if (koniecDialogu.npc_id) {
                    ...
                    if (typeof AjaxHelper !== 'undefined' && AjaxHelper.sendRequest) {
                        // Ustal URL i token bezpieczeństwa
                        const ajaxUrl = ajaxConfig?.ajaxurl || window.ajaxurl || '/wp-admin/admin-ajax.php';
                        const securityToken = ajaxConfig?.security || '';

                        AjaxHelper.sendRequest(ajaxUrl, 'POST', {
                            action: 'get_npc_popup',
                            npc_id: npcId,
                            page_id: JSON.stringify(pageData),
                            current_url: window.location.href,
                            security: securityToken // Dodajemy token bezpieczeństwa
                        })
                            .then(response => {
    ...
    function getSceneIdFromUrl() {
        ...
        // Jeśli ścieżka jest pusta, zwróć 'main'
        if (pathSegments.length === 0) {
            return 'main';
        }

        // Iteruj po segmentach ścieżki w odwrotnej kolejności (od końca), 
        // aby znaleźć pierwszy sensowny segment
        for (let i = pathSegments.length - 1; i >= 0; i--) {
            const lastSegment = pathSegments[i];

            // Ignoruj typowe końcówki URL
            if (lastSegment === 'index.php' || lastSegment === '/') {
                continue;
            }

            // Ignoruj ID numeryczne (prawdopodobnie ID postów)
            if (/^\d+$/.test(lastSegment)) {
                continue;
            }

            // Jeśli trafimy na segment z rozszerzeniem, usuń je
    ...
    function loadDialogsFromPostId(postId, callback) {
        if (!postId) {
            ...
            return;
        }        // Obiekt przechowujący dane konfiguracyjne AJAX
        const ajaxConfig = {};

        // Sprawdź różne możliwe źródła konfiguracji i tokenu bezpieczeństwa
        if (typeof window.npcDialogsData !== 'undefined') {
            // Oryginalny obiekt npcDialogsData
            ajaxConfig.ajaxurl = window.npcDialogsData.ajaxurl;
            ajaxConfig.security = window.npcDialogsData.security;
        } else if (typeof window.global !== 'undefined' && window.global.ajaxurl) {
            // Obiekt global używany w nowej strukturze kodu
            ajaxConfig.ajaxurl = window.global.ajaxurl;
            ajaxConfig.security = window.global.npc_dialog_nonce || '';
        } else if (typeof window.ajaxurl !== 'undefined') {
            // Zmienna globalna ajaxurl
            ajaxConfig.ajaxurl = window.ajaxurl;
            ajaxConfig.security = window.npc_dialog_nonce || '';
        } else {
            // Awaryjnie używamy domyślnego URL do WordPress AJAX
            ajaxConfig.ajaxurl = '/wp-admin/admin-ajax.php';
            ajaxConfig.security = '';
            console.warn('Brak danych AJAX i tokenu bezpieczeństwa dla dialogów NPC. Może wystąpić błąd weryfikacji.');
        }
        ...
        formData.append('security', ajaxConfig.security);
    ...
    /**
     * Wyświetla dialog dla NPC
     * @param {string} npcId - ID NPC
     * @param {string} message - Treść dialogu
